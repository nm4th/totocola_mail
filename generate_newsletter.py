...
# 週4本のスケジュール（月・火・木・土）
# 配分:
#   件名型      : reason×2, statement×1, question×1
#   プレビュー型: A×2, B×1, C×1（C は少量で検証継続）
#   テーマ方向  : 内側×2, 夜の行動×1, 人間関係×1
#   長さ        : short×3, long×1（短めが高開封率）
WEEKLY_SCHEDULE = [
    {
        "offset": 0, "weekday": "月",
        "subject_type": "reason",    "preview_type": "A",
        "theme_direction": "自分の内側・感情系", "length": "short",
    },
    {
        "offset": 1, "weekday": "火",
        "subject_type": "statement", "preview_type": "B",
        "theme_direction": "夜の行動・習慣系",   "length": "short",
    },
    {
        "offset": 3, "weekday": "木",
        "subject_type": "question",  "preview_type": "A",
        "theme_direction": "人間関係・他者との距離系", "length": "short",
    },
    {
        "offset": 5, "weekday": "土",
        "subject_type": "reason",    "preview_type": "C",
        "theme_direction": "自分の内側・感情系", "length": "long",
    },
]
...
- 「〜しましょう」
- 商品スペックの列挙
- 送り手（ととコーラ）の日記・体験談にならないこと
- ととコーラの名前は本文中1〜2回まで

必ずJSONのみ返してください。```や説明文は不要です。"""


def build_user_prompt(slot: dict, used_themes: list, sample_text: str) -> str:
    """Claude APIへのユーザープロンプトを構築する。

    slot は WEEKLY_SCHEDULE の1要素。subject_type / preview_type /
    theme_direction / length の4軸を渡す。
    """
    themes_str = "\n".join(f"- {t}" for t in used_themes)

    return f"""以下の条件でメルマガを1本生成してください。

件名の型: {slot['subject_type']} （reason=〜理由型 / statement=断言型 / question=疑問形）
プレビューの型: {slot['preview_type']} （A=答えを匂わせる / B=共感を重ねる / C=問いかけで終わる）
テーマの方向性: {slot['theme_direction']}
件名の長さ: {slot['length']} （short=20文字以内 / long=21文字以上）
...
emphasisは各セクションで最も伝えたい一文を指定。なければ空文字。"""


def call_claude_api(api_key: str, slot: dict, used_themes: list, sample_text: str, max_retries: int = 3) -> dict:
    """Claude APIを呼び出してメルマガコンテンツを生成する。

    slot は WEEKLY_SCHEDULE の1要素。
    一時的なエラー（429, 500, 502, 503）はリトライする。
    クレジット不足などの永続的エラーは即座に停止する。
    """
    user_prompt = build_user_prompt(slot, used_themes, sample_text)
...
<html lang="ja" style="background-color:#1a1f2e;">
<head>
<meta charset="UTF-8">
...
<p style="margin:0;font-size:13px;color:#8b92a8;letter-spacing:0.12em;">{subtitle}</p>
</td></tr>

<!-- 冒頭ブロック -->
<tr><td style="padding:24px;">
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#252b40;border-radius:4px;">
<tr><td style="padding:24px 28px;font-size:18px;line-height:1.6;color:#c8cdd8;">
{opening_html}
</td></tr>
</table>
</td></tr>
...
def generate_week(start_date: date) -> None:
    """1週間分（4本: 月・火・木・土）のメルマガHTMLを生成する。"""
    ...
    for index, slot in enumerate(WEEKLY_SCHEDULE, start=1):
        delivery_date = start_date + timedelta(days=slot["offset"])
        date_str = delivery_date.strftime("%Y-%m-%d")
        weekday = slot["weekday"]
        next_dates[weekday] = date_str

        print(
            f"\n[{index}/{total}] {date_str}（{weekday}） "
            f"件名:{SUBJECT_TYPE_LABELS[slot['subject_type']]} / "
            f"プレビュー:{slot['preview_type']}({PREVIEW_TYPE_LABELS[slot['preview_type']]}) / "
            f"方向:{slot['theme_direction']} / 長さ:{LENGTH_LABELS[slot['length']]}"
        )
        ...
        # ファイル名を生成: YYYY-MM-DD_曜日_件名.html
        subject_clean = sanitize_filename(subject)
        filename = f"{date_str}_{weekday}_{subject_clean}.html"
        ...
        # 件名リスト用データを蓄積
        subject_list_lines.append(f"{date_str}（{weekday}）")
        subject_list_lines.append(f"件名：{subject}")
        subject_list_lines.append(f"プレビュー：{preview}")
...
def main():
    ...
    print(f"\n配信開始日: {start_date}（月曜日基準）")
    print(f"テンプレート参照: {TEMPLATES_DIR}")
    print(f"配信スケジュール: 月・火・木・土の週4本")
    print()
